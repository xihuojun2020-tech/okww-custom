# 账号总配置运行端 AI 详细说明（v1.07.00）

本文供运行端的 AI 工具、维护脚本和人工维护者理解账号总配置的安全边界。除用户在完整性弹窗中明确确认的“旧版本首次锚定”外，主程序不会创建或修改总配置。AI 在操作前必须先向用户说明影响并备份文件，不能替用户点击确认。
...
| 路径 | 职责 | 主程序是否写入 |
| --- | --- | --- |
| `configs/account_master_config.json` | 所有账号的稳定意图和唯一可信来源 | 正常运行时只读；仅用户确认的首次锚定事务可创建一次 |
| `configs/daily_profiles.json` | 兼容旧版界面和任务代码的工作副本 | 首次锚定时可补齐稳定 ID/缺失默认值；用户选择总配置覆盖时可重建 |
