...
适用当前 1.84.01。本文供运行端的 AI 工具、维护脚本和人工维护者理解账号总配置的安全边界。普通任务和普通配置保存不会创建或修改总配置；只有用户明确确认的旧版本首次锚定、遗漏序列恢复、账号配置包导入或完整备份恢复可以进入受控写事务。历史迁移章节中出现的 1.08.00 是功能首次引入版本，不是运行端更新目标。

当前账号页将任务设置分为“日常与声骸”“周常安排”等区域；“待办提醒”固定六项：活跃度、周本、每周乐园、深渊、活动和其他，并提供账号备注。待办与完成检查各自独立，勾选待办不会启动任务，也不会修改自动任务断点。每周乐园检查日选“无”代表该账号禁用，包括周日；若需周日检查须显式选“星期日”。
