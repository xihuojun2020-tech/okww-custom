...
> 目标版本：okww-custom（当前应已更新到 **v1.04.0**）
...
## 5. 任务 E：更新到 v1.04.0 并运行连续账号切换测试（当前任务）

### E1. 更新
1. 通过 GitHub/PyAppify 更新到 **v1.04.0**；
2. 确认 `working\config.py` 为 `version = "1.04.0"`；
...
6. 若当前显示账号与目标不一致，应看到“重新选择目标账号”日志，而不是第一次不一致就停止；只有多次重试仍不一致才会为防误登而停止。

### E3. 报告
```text
- v1.04.0 连续测试结果：通过 / 仍失败
- 实际切换顺序：A1 → A3 → A4 / 其他
- 是否发生重新选择：否 / 是（第几个账号）
