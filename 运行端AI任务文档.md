...
> 文档版本：2026-08-18 v2（开发端提供）
> 目标版本：okww-custom（当前应已更新到 **v1.04.1**）
...
## 5. 任务 E：更新到 v1.04.1 并运行连续账号切换测试（当前任务）

### E1. 更新
1. 通过 GitHub/PyAppify 更新到 **v1.04.1**；
2. 确认 `working\config.py` 为 `version = "1.04.1"`；
...
6. 若当前显示账号与目标不一致，应先看到“等待界面稳定/界面闪烁”日志；目标账号连续识别两次后确认。稳定等待超时才进入“重新选择目标账号”流程，而不是第一次不一致就停止；只有多次重试仍不一致才会为防误登而停止。
7. 点击账号后登录器列表可能短暂展开或 OCR 为空，这是游戏界面切换过渡态；应等待列表收起和账号稳定，不要手动中断。

### E3. 报告
```text
- v1.04.1 连续测试结果：通过 / 仍失败
- 实际切换顺序：A1 → A3 → A4 / 其他
- 是否发生重新选择：否 / 是（第几个账号）
