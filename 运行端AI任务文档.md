# 运行端 AI 任务文档：多账号每日任务与诊断

> 面向：运行端（okww 使用端机器）上的 AI 助手
> 当前源码版本：**1.84.01**（以运行设备的 `config.py` 和程序界面为准）
> 下方 0—6 节及探针附录是 2026-08-19 至 v1.05.01 的故障调查记录，不是今天的待执行任务。

## 当前运行端步骤

1. 使用 [其他设备更新操作文档](docs/其他设备AI私有仓库更新操作文档.md)确认本机实际版本、安装目录和账号配置。不要把此文历史段落中的 v1.03.xx 或 v1.08.00 当作更新目标。
2. 在账号总配置中核对账号 UUID、别名、序列与各账号每日任务设置；“待办提醒”及备注仅供人工查看，不会安排自动任务。每周乐园日期为“无”时，该账号禁用乐园任务，包括周日。
3. 如果需要单独排查账号切换，运行 `TestAccountSwitchTask` 的连续序列模式，默认按精确短名 A1、A3、A4。该测试复用正式多账号任务的选择、别名核对、登录、退登与重试方法；按设备实际备用登录名及掩码手机号核验，不执行每日任务。
4. 若真实每日任务失败，记录账号短名、任务阶段、时间及错误。到“工具 → 日志与诊断”手动上传当天日志和截图；此前未手动上传的日期会在次日首次启动时逐日补传。界面显示打包、连接、上传进度、速度和结果。NAS 唯一路径与保留规则见 [NAS 诊断说明](docs/NAS诊断部署与读取说明.md)。
5. 账号配置完整性、导入与恢复依据下文 7—8 节及 [账号总配置详细说明](账号总配置运行端AI详细说明.md)操作；历史探针仅在重新遇到对应登录窗口故障时作为参考。
...
## 5. 历史任务 E：更新到 v1.05.01 并运行连续账号切换测试
...
## 7. 账号总配置完整性与迁移（v1.08.00）

账号稳定意图现在由 `configs/account_master_config.json` 提供。普通任务和配置界面对该文件只读，
不会静默把当前界面或工作副本写回总配置；AI、编辑器或其他外部工具仍可正常修改该文件。主程序只有在用户明确确认首次锚定、遗漏序列恢复、账号配置包导入或完整备份恢复时，才执行受控事务。
旧的 `configs/daily_profiles.json` 仅作为兼容工作副本。
...
5. 若总配置已经生成但序列为空，而 `MultiAccountDailyTask.json` 仍保存旧序列，v1.08.00 会在自动任务启动前提示恢复。用户确认后应恢复序列一、序列二及其成员顺序；拒绝不会写文件，可稍后在设置页选择“恢复旧版遗漏序列”。已有非空总序列不会被旧文件覆盖。
...
## 8. v1.08.00 导入导出、备份与失败证据

1. “导出账号配置”生成 `okww_account_bundle` v2 JSON，其中总配置、UUID 运行记录和非权威偏好分别保存，并由 `manifest.partitions` 记录 SHA-256。运行端 AI 可读取该文件，但修改后导入会明确要求用户信任外部修改。
2. “导入账号配置”先做纯内存预检。出现非法 UUID、别名歧义、悬空序列、未知格式或缺失校验值时必须停止；AI 不得绕过。用户确认后程序先制作事务快照，再替换总配置、重建工作副本、按 UUID 恢复运行记录并复检。本机接受指纹、事件绝对路径和上次导入标记不随配置包迁移；旧 v1 的账号内完成时间会转换到 UUID 运行记录。
3. 每日完整备份位于配置的备份目录，包含整个 `configs` 和 `manifest.json`。默认保留每日 30 份、事务 20 份、总量 2GB；不得拆开某个快照复制部分文件冒充完整备份。
4. “恢复备份”预检会报告总配置账号数、序列数和成员数量；存在总配置但完整性不通过的快照不得恢复。恢复会替换整个配置树，暂存校验或最终复检失败时应立即换回原目录；成功后必须重启程序再继续任务。若看到 `.restore-journal.json`，说明目录交换曾中断，不要删除 journal、staging 或 rollback 目录；先让程序执行受限恢复或交由测试端分析。
5. 账号切换失败证据位于 `screenshots/account_switch_failures/`。每个事件含 `event.json` 和最近关键帧，红框表示 OCR 目标，红色十字/圆表示尝试或实际点击点，文字记录投递方式、重试轮次、窗口坐标和屏幕坐标。
6. 失败证据保留 7 天、最多 20 次、总量 500MB，程序启动及每 6 小时清理最旧完整事件。成功切换的内存帧不会写盘。

报告问题时优先提供失败事件目录和脱敏日志，不要只截取最后一张图；`event.json` 的 `delivered`、`attempted_point`、`actual_click_point`、`window_point`、`screen_point` 可用来判断识别、坐标换算或游戏未接收点击。
