...
> 目标版本：okww-custom（当前应已更新到 **v1.04.02**）
...
## 5. 任务 E：更新到 v1.04.02 并运行连续账号切换测试（当前任务）

### E1. 更新
1. 通过 GitHub/PyAppify 更新到 **v1.04.02**；
2. 确认 `working\config.py` 为 `version = "1.04.02"`；
...
7. 如果登录器没有接受 PostMessage 点击，应先刷新窗口句柄并重投；连续两次投递仍未确认时，程序只会在列表仍展开且目标 OCR 框仍可验证的前提下使用系统屏幕点击兜底。任何点击方式都不能绕过登录前账号核对。
8. 日志应区分“已发送账号点击”与“确认已选择账号”，并记录点击方式、目标 HWND/窗口类和坐标；只有多次重试仍不一致时才会为防误登停止。

### E3. 报告
```text
- v1.04.02 连续测试结果：通过 / 仍失败
- 实际切换顺序：A1 → A3 → A4 / 其他
- 是否发生重新选择：否 / 是（第几个账号）
