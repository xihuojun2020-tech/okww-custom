    ...
    def _add_update_log_card(self):
        try:
            ...
            update_log_text.setPlainText(upstream_note + self.tr(
                'V1.19.00：普通账号编辑锁定七类身份字段，新增校验当前身份、冲突检查、备份和 CAS 发布的独立重新绑定流程；账号页身份字段只读，身份变更不混入任务配置保存；本机备份使用 Windows DPAPI，完整账号快照写入 .dpapi 旁车文件，备份根目录默认启用当前用户 ACL，恢复拒绝路径遍历和符号链接逃逸；运行时开始统一读取 active bundle 图存储。\n'
