    ...
    def _add_update_log_card(self):
        try:
            ...
            update_log_text.setPlainText(upstream_note + self.tr(
                'V1.33.00：账号、序列和备份维护写盘转入后台，保留草稿及版本冲突保护；切号证据采样先过滤并限制原始帧缓冲为128 MiB，完成2/10/50账号规模和4K帧测量。\n'
                'V1.32.01：统一备份目录并支持跨盘恢复和启动中断恢复，修复清理无进展、月卡小时范围、嵌套配置包检查及角色代码连续替换；运行中角色修改延后生效。\n'
                'V1.32.00：统一账号导入、编辑和删除事务及启动恢复，固定整轮执行快照；修复备用名停用、测试页账号列表和超过10个序列的问题，减少重复账号图校验，增强离线测试超时与进程隔离。新增全面代码审查和结构说明。\n'
                'V1.31.15：更新测试功能中的多账号账号切换链路，连续切换复用正式生产流程并在结束后按正式流程登录回起始账号，新增离线集成回归测试。本次未启动或操控游戏。\n'
                'V1.31.14：修复账号切换稳定检测因下拉框动画抖动导致的误判；目标账号已识别时不再重复打开列表，新增离线回归测试。本次未启动或操控游戏。\n'
                'V1.31.13：按原版账号切换思路减少重复置前窗口和等待；整屏 BitBlt 短暂空帧/纯色帧时回退 WGC，进程不可信时仍严格拒绝。保留独立登录窗口的 SendInput 和账号安全校验，新增相关离线回归测试。本次未启动或操控游戏。\n'
                'V1.31.12：修复凝素领域战斗结束后因切人瞬间退出战斗，仍强制寻找寻宝目标并在30秒后抛出 WaitFailedException 的问题；战斗状态已清除且没有目标/敌人血条时跳过寻宝继续结算，真正死亡仍走复活分支。新增领域战斗收尾与寻宝超时回归测试，本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.11：修复账号下拉列表 ComboLBox 弹出窗口位于父窗口外时被 SendInput 边界校验拒绝的问题；BitBlt 账号切换遇到独立登录窗口句柄替换时会刷新 HwndWindow 快照并重新收集可信 PID；兼容 OCR 少识别掩码手机号星号。新增下拉弹窗、句柄刷新和 OCR 容错回归测试，本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.10：修复 ESC 后终端菜单被误判为未知状态的问题：设置模板和左下角电源图标均可确认设置页，并复用同一张 BitBlt 帧执行点击；未确认退出流程进入登录阶段前不再对未知帧执行整屏登录 OCR，避免单次 OCR 阻塞退登窗口；新增状态耗时和截图来源诊断日志。本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.09：修复残象聚落战斗中角色阵亡、通用复活传送失败后整个梦魇任务直接终止的问题。现在会等待恢复到大世界，重新打开F2并重新扫描各聚落实际n/N进度，只补打未完成的当前聚落；保留连续三次进度不增长的停止保护，进程丢失、截图中断和配置损坏仍会立即停止。新增阵亡恢复离线回归测试，没有启动或操控游戏。\n'
                'V1.31.08：修复多账号任务错误拒绝110～180活跃度、账号列表已经展开却被连续帧OCR否决，以及组合登录窗口切换句柄或进程后被误判为无关前台程序的问题。活跃度现在接受0～180；同帧出现账号身份和登录按钮即确认登录页，已展开列表直接进入目标账号识别；Foreground BitBlt在单次切换中信任窗口管理器发现的游戏登录组合窗口，并使用实际前台窗口及其PID完成强制前台和SendInput安全校验。新增对应离线回归测试，没有启动或操控游戏。\n'
                'V1.31.07：修复自动深渊已经正确选择三名角色，却因旧编号OCR覆盖体力区域而把视觉编号2误读成1的问题。正常编队不再识别编号文字，改为检查已点击角色卡两侧稳定的金色选择边框，并按点击顺序确定1/2/3；每次点击和最终确认只检查目标角色的局部标记区域，清理旧编队复用已有扫描并只检查卡片边缘，局部失败时记录全页诊断位置、保存截图且不重复点击。新增1280×720至3840×2160选择边框测试和本次2560×1440真实选择标记回归样本。本次仅执行离线测试，没有启动或操控游戏。\n'
