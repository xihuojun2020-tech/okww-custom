    ...
    def _add_update_log_card(self):
        try:
            ...
            update_log_text.setPlainText(upstream_note + self.tr(
                'V1.31.06：修复自动深渊把体力为0的角色误判为10并选入队伍的问题。体力识别现在结合黄色轮廓确认闪电图标后实际有一位还是两位数字，能够区分“⚡0”和真实 10；轮廓或 OCR 无法相互确认时按不可用处理并保存诊断截图，不再冒险编队。新增取自失败画面的0与10体力裁剪图回归测试。本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.05：修复自动深渊重新编队时角色列表偶发无法滚动的问题：每次滚轮输入前都强制前台并等待页面稳定，两次滚轮无效后拖动右侧滚动条兜底，同时记录网格变化、滚动条位移及失败前后截图。全部滚动方式失效时，仅当第一屏至少有三名等级大于60且体力满足当前关卡消耗的角色才继续编队，否则明确停止。本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.04：修复自动深渊楼层入口的整屏 OCR 在 2560×1440 下漏识别、导致编队完成后无法点击“开启挑战”的问题。现在使用同一帧分别在左侧提示区、左上标题区和右下按钮区识别“环境特性”“编辑队伍”和“开启挑战”，保留逐层点击与已加载时跳过重复点击的流程。新增局部区域和同帧调用回归测试。本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.03：修复自动深渊点击“继续挑战”后停在下一层编辑队伍页的问题：每一层都会确认当前处于编辑队伍或环境特性阶段；识别到编辑队伍与“开启挑战”后执行点击，已经进入环境特性加载阶段则跳过重复点击。新增连续楼层和直接加载回归测试。本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.02：修复自动深渊将已选中的第三层误判为状态未知：低亮度选中边框使用相对最强行与最低分联合判断，“重置/挑战开始”改为独立高置信证据。编队只按当前可挑战关卡计算体力门槛；当前队伍不足下一层时返回深塔重新扫描并重新编队，避免中塔错误要求单名角色具备20点体力。本次仅执行离线测试与打包版2560×1440失败截图验证，没有启动或操控游戏。\n'
                'V1.31.01：修复每日活跃度最终校验在领取前抛错的问题：程序现在始终先领取已达到的奖励，再判断账号是否完成。活跃度 OCR 连续读取三张新帧，只接受 0～100 的纯数字；识别不到时标记为未知，不再伪装成 0，并在未知状态禁止使用备用体力。领取按钮优先识别“领取/領取/Claim”，找不到时保留坐标兜底，按钮仍存在时最多重试一次；领取后明确未满与识别失败分别提示。本次仅执行离线测试，没有启动或操控游戏。\n'
                'V1.31.00：完善多账号每日任务的失败隔离与体力策略：梦魇聚落按 n/N 部分进度继续执行并检测连续无增长；游戏窗口或截图丢失使用专用异常，单账号失败保留断点、最多受控重启游戏一次、继续后续账号并最终回登起始账号；登录账号下拉列表要求连续两张新帧确认。活跃度已满时不使用备用体力并清理现有体力至不足单次消耗，未满时无音区/其他按180、凝素领域按200预算决定是否使用备用体力，预算不足则只使用现有体力，仍未满时提醒且不标记账号完成。自动战斗以目标或血条任一证据继续，避免敌人仍存活时过早结束。代理自愈同步清理遗留 https.proxy，避免失效旧端口继续阻断 GitHub。本次仅进行离线测试，未启动或操控游戏。\n'
