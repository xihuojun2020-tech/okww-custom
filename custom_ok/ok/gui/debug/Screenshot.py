...
class Screenshot(QObject):
    ...
    def __init__(self, exit_event, debug):
        ...
        if self.click_screenshot_folder is not None or self.screenshot_folder is not None:
            ...
            # ExitEvent.put(None) can deadlock a full bounded queue after the
            # worker sees exit_event. Use our nonblocking stop callback instead.
    ...
    def add_task(self, frame, folder, name=None, show_box=False, frame_box=None):
        if self.task_queue is not None:
            # Ordinary debug screenshots only. Material/required completion
            # repositories have their own durable, non-dropping save path.
            size = frame.nbytes
            with self._bytes_lock:
                if self._queued_bytes + size > 128 * 1024**2:
                    logger.warning('Debug screenshot byte budget reached; skipped ordinary screenshot')
                    return
                self._queued_bytes += size
            try:
                self.task_queue.put_nowait(
                    (frame.copy(), self.ui_dict.copy(), folder, f'{get_current_time_formatted()}_{name}', show_box, frame_box))
            except queue.Full:
                with self._bytes_lock: self._queued_bytes -= size
                logger.warning('Debug screenshot queue full; skipped ordinary screenshot')
    ...
    def _worker(self):
        while True and not self.exit_event.is_set():
            ...
            try:
                ...
                logger.error(f'Debug screenshot save failed: {type(error).__name__}')
            finally:
                with self._bytes_lock: self._queued_bytes -= task[0].nbytes
                self.task_queue.task_done()
    ...
    def stop(self):
        ...
        if self.task_queue is not None:
            try: self.task_queue.put_nowait(None)
            except queue.Full: pass  # exit_event also terminates the worker.
