...
class StartTab(Tab):
    def __init__(self, config, exit_event):
        ...
        self.update_log_text.setPlainText(upstream_note + self.tr(
            ...
            '\n'
            'V1.03.13：更新日志补充 v1.03.08~v1.03.12（打包发布修复记录）\n'
            '\n'
            'V1.03.12：同步全部定制 ok 框架文件（LabelAndLabel 等 8 个），修复启动模块缺失\n'
            '\n'
            'V1.03.11：启动时自动同步定制 ok 框架（ConfigItemFactory 支持项目级配置组件）\n'
            '\n'
            'V1.03.10：升级 ok-script 1.0.185 → 1.0.190（修复配置类型兼容）\n'
            '\n'
            'V1.03.09：修复 src/__init__.py 未提交（gitignore 误匹配 __init__.py）\n'
            '\n'
            'V1.03.08：任务后录像时长改为 1.5 秒\n'
            '\n'
            'V1.03.07：修复任务后录像页面（残像聚落改用首领书正确入口、大月卡改用左上角战令入口）\n'
            '\n'
            'V1.03.06：移除序列切换功能（登录器记住账号在服务器端，本地切换无法绕开 10 个限制）；'
            '多账号任务保留（序列仅作分类标识）\n'
            '\n'
            'V1.03.05：游戏账号标记（Binaries\\Win64）纳入序列隔离，切序列后游戏不再显示旧账号\n'
            '\n'
            'V1.03.04：修复游戏登录缓存切换失败（联接点删除改用系统专用方法）\n'
            '\n'
        ...
        update_note_widget = QWidget()
        update_note_layout = QVBoxLayout(update_note_widget)
    ...
    def add_log_panel(self, level_no, message):
