...
class StartTab(Tab):
    def __init__(self, config, exit_event):
        ...
        self.update_log_text.setPlainText(self.tr(
            ...
            '多账号任务保留（序列仅作分类标识）\n'
            '\n'
            'v1.03.05 更新：游戏账号标记（Binaries\\Win64）纳入序列隔离，切序列后游戏不再显示旧账号\n'
            '\n'
            'v1.03.04 更新：修复游戏登录缓存切换失败（联接点删除改用系统专用方法）\n'
            '\n'
            'v1.03.03 更新：更新日志改为可滚动的历史记录（所有版本保留）\n'
            '\n'
            'v1.03.02 更新：修复启动崩溃（QLabel 未导入、开关配置类型不存在）\n'
            '\n'
            'v1.03.01 更新：多账号每日任务（断点续跑/账号序列/扫码账号识别）、'
