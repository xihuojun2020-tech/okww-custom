...
class MessengerAutomation:
    ...
    def _wait_until_background(self, hwnd):
        # 最多等待 10 秒让 QQ/微信让出前台；超时后照常执行。
        # PostMessage 交互不抢占前台，后台/前台窗口均可安全操作，
        # 避免无人值守托管场景下 QQ 一直前台导致通知卡死。
        deadline = time.monotonic() + 10
        while win32gui.GetForegroundWindow() == hwnd and time.monotonic() < deadline:
            if self._should_stop():
                return False
            time.sleep(.25)
        return True
    ...
    def _header_region(self, hwnd):
        ...
        QQ NT 版聊天标题显示在窗口顶部（可能偏左），需要全宽扫描顶部区域；
        微信标题在右侧面板顶部。统一扫描整行顶部即可覆盖两种布局。
        """
        _, _, width, height = win32gui.GetClientRect(hwnd)
        try:
            scale = ctypes.windll.user32.GetDpiForWindow(hwnd) / 96.0
            ...
            scale = 1.0
        return 0, 0, width, min(height, round(140 * scale))
