...
try:
    ...
    # 兼容：较新 ok-ww 源码依赖的 Icon 在已发布 ok-script 中不存在，
...
}, description='Config auto backup on first launch of each day', config_description={
    'Config Backup Directory': 'Leave empty to use the default: <project>/configs_backup',
})

# 数据仓库（ok仓库）：所有备份/输出文件统一存放（监控录像/配置备份/账号数据）
...
config = {
    ...
    'windows': {  # required  when supporting windows game
        ...
        # 点「开始」任务时不自动启动游戏本体（游戏/登录器由用户手动打开，配合序列切换流程）
        'start_exe': False,
        'check_hdr': False,
        'force_no_hdr': False,
...
""",
    ...
    'onetime_tasks': [  # tasks to execute
        ...
        ["src.task.MaterialPlannerTask", "MaterialPlannerTask"],
        ...
        ["src.task.EchoesRemainTask", "EchoesRemainTask"],
        ["src.task.ResonanceSimulationTask", "ResonanceSimulationTask"],
        ["src.task.CharacterTrialTask", "CharacterTrialTask"],
