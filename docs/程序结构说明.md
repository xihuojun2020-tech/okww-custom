...
已知边界：`_fight()` 当前只把 `CombatStateUnknown` 纳入上述复核。`BaseCombatTask.combat_once()` 在战斗循环之外执行战前/战后切治疗时，队伍界面因过场消失可能直接抛出 `NotInCombatException`；该异常尚未进入周本阶段复核，仍可能以 `not in_team while switching` 中止。后续补丁应在周本局部同时捕获这两种可复核脱战异常：确认仍为战斗阶段时等待目标并继续，确认战后时寻奖，未知超时则重新抛出原异常。`CharDeadException`、停止、游戏/截图丢失和配置错误继续直接传播，不修改公共战斗基类。
