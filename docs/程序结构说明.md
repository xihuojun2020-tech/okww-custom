...
审查基线：`3fd591a5` / `1.31.15`；本文已同步整改批次 A/B/C/D、首次安装启动及快捷键/窗口切换诊断修复（`1.33.03`），更新日期：2026-09-07。
...
规模按 1.33.01 整改后源码中的 Python 文件统计，不包含 `.venv` 和本地生成文件。文件行数用于定位维护重心，不作为质量评分。
...
2. `_sync_custom_ok()` 将项目维护的 12 个框架文件复制到安装环境。`ok/util/window.py` 保留框架窗口工具，针对采样期间窗口销毁的错误码 1400 返回无效尺寸并等待下次发现；其他异常继续记录错误。
