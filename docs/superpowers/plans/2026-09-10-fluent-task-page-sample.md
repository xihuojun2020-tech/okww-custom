...
- [x] **Step 2: 同步当前覆盖并运行测试，确认失败于未知构造参数。**
...
- [x] **Step 3: 在 TaskCard 构造末尾实现局部布局。** 保留原方法及状态绑定，仅在参数为 true 时执行：
...
- [x] **Step 4: 扩展现有独立点击测试，在普通和样板两种构造参数下分别运行。**
...
- [x] **Step 1: 检查所有 TaskCard 和 OneTimeTaskTab 调用，确认可选参数不破坏旧调用。**

```powershell
rg -n 'TaskCard\(|OneTimeTaskTab\(' src custom_ok tests
```

- [x] **Step 2: 保存并传递样板参数，在 TaskHubTab 开启。**
...
- [x] **Step 1: 为 renderer 添加可选 `OKWW_UI_SAMPLE_STATE`，仅在模拟任务列表构造后注入任务状态。**
...
- [x] **Step 2: 用真实控件渲染默认、展开、运行及错误状态；每次清理本次设定的环境变量。**
...
当前任务内手工执行完成阶段一样板及离线验收。代码复用了既有颜色 token，额外补齐仅说明卡片的空箭头隐藏与按钮对齐；未修改 ConfigCard/DisclosureHeader 共用行为。全量 1030 项（1022 通过、8 跳过），最终按钮对齐后 UI 18 项再次通过。60 组布局检查通过，最终收起/展开截图已复查。发布结果以本任务最终 Git 远端核验为准，用户视觉确认尚未完成。
