...
> 2026-09-12 实施状态：用户已授权实现、文档更新及 GitHub/NAS 发布，并明确由用户自行实机验收。核心数据、字典、扫描/刷取协调、永久结算、统计导出、NAS别名回退已实现，默认关闭材料自动规划。1200项隔离回归无失败（8项既有跳过）；后续小改动另跑相关测试。原计划下方未勾选项保留为细项/实机验收追踪，不代表所有条目均已完成。实际接口及边界以[使用说明](../../references/material-planner-1.59.md)和[验收记录](../../reviews/2026-09-12-material-planner-acceptance.md)为准。

| 工作 | 当前状态 |
|---|---|
| 15张脱敏样本、40凝素模板、7怪物排除模板 | 完成；其余怪物组合待补样本 |
| 四档单向合成、绿色格数统计、周标记 | 完成并测试 |
| SQLite、不可变原图、解析修订、CSV与备份 | 完成并测试 |
| 两图22格真实OCR拼接、十组入口预览 | 离线回放通过 |
| 仓库周校准、培养页刷新、凝素/声骸调度 | 实现及契约测试通过；用户手动实机验收 |
| 周本自动首项及既有关闭兼容 | 实现并回归 |
| NAS 172/173、新共享名、凭据别名 | 完成；172实际访问通过，173故障回退用模拟测试 |
| GitHub与NAS发布 | 以验收记录中的发布回读结果为准 |
| 均值预测控制、其他实际奖励份数与难度样本 | 未启用/待后续样本 |
...
- [x] 检查设计第8节的15张原图全部存在，保留原图不动，逐张确认尺寸。产出脱敏回放副本，遮住右下账号标识和无关信息。
- [x] 标注十组四十档模板，组a/b依据实际图标定义，不按副本名字命名。每个模板避开数量、新标记、头像、选中框；无法完整裁切的图标要求新的真实样本，不能用截断模板充当完整模板。
...
- [x] 用给定两张实际结算跑完整OCR/图标/拼接管线，核对22格、目标2绿色格、13/16/3/0、88。单纯注入人工OCR的用例不能替代真实回放。
...
- [ ] 逐个运行定向测试后进行项目完整测试：`./run_tests.ps1 -Group all`（逐文件隔离，避免Qt单例冲突）。执行前确认该命令与届时仓库测试入口兼容；记录已有失败和环境依赖，不能把未执行记通过。
- [ ] 验收文档逐项写“通过/失败/未验收”及证据路径、提交SHA、识别版本和样本数，不能以大量合成图代替游戏真实交互。
- [ ] 完成实现且验证后，按届时版本递增次版本并归零补丁号（若仍1.58.00则1.59.00），同步产品版本和说明。按deploy技能核对发布remote，创建匹配注释标签并推送分支与标签；用户若要求本地则不推。用户于2026-09-12选择自行实机验收：发布时保持默认关闭，明确标注未实机验收项，不宣称自动消费路径已实机通过。
