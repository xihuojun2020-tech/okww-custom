...
**执行进展：** 批次 A（T00–T06）已发布为 `1.32.00`；批次 B（T07–T11）已实现并完成重点离线验证，版本 `1.32.01`，安装器实装验收仍归 T15；批次 C（T12–T14）已完成规模测量、证据缓冲及 GUI 后台写盘，版本 `1.33.00`；详见性能测量记录。具体实现调整、测试计数和验证边界见[整改执行记录](../../reviews/2026-09-06整改执行记录.md)。以下清单保留原始验收细目，完成结论以执行记录的实际证据为准。
...
- [x] `list_profiles()` 从一次 `_load_index()` 得到 raw/accounts/sequences，直接构造全部 ProfileRecord。`legacy_profile_projection()` 在同一组数据上同时生成 profiles、显示名映射和 sequences。
- [x] `get_detached_projection` 保持旧别名/调用方式兼容；每个返回值都是独立副本，调用方修改不会污染仓库或同轮其他账号。
- [x] 快照创建使用一次已校验图，避免先列账号再按账号单独重读。需要复用的内部构造逻辑留在仓库模块，不新增永久缓存服务。
- [x] 先实现操作内复用；跨操作缓存只有在后续测量仍显示必要时才考虑。不同操作仍发现 active 改变或 bundle 损坏，不能以缓存提升速度为由跳过完整性检查。
- [x] 用 T00 的真实发布环境统计校验调用，基线测试先失败，修复后每次投影不超过一次完整校验。
...
- [x] 用 2、10、50 个合成账号记录 wall time 和校验次数，检查次数不随 N 增长；用例不以某台机器的绝对毫秒作为通过门槛。
- [x] 在两次调用之间修改 active、损坏文件、修改返回对象，分别验证新版本可见、损坏被拒绝、结果互不污染。
...
- [x] 在 `frame.copy()` 前检查会话结束、空图像、采样时间和单帧大小；普通被丢弃采样不发生复制。
- [x] 显式维护缓冲总字节数；时间淘汰、数量淘汰、容量淘汰和 finish 都同步减账。不能继续依靠 deque 自动弹出却不知道弹出了多少字节。
- [x] 强制关键帧可以绕过时间间隔，不能绕过总内存上限。单帧超过上限则只记录事件和“帧超限未保留”的元信息，避免为记录失败现场本身耗尽内存。
- [x] 用可计数图像对象验证跳过采样不复制：
...
- [x] 测试中把字节上限调小，用 NumPy 小数组验证普通帧、强制帧、过期帧的字节上限；重复 finish 不产生负计数。再用 4K 合成帧测量峰值，报告同时说明临时输入帧和编码副本不在该缓冲上限内。
...
- [x] 测量账号数量 2/10/50 时保存、创建、序列更新的耗时；将校验、备份、ACL、写盘分别计时，不猜测 OCR 是原因。
- [x] 使用现有 Qt 工作者机制或 PySide6 自带 QThreadPool/QRunnable 执行非 UI 工作。提交前在 GUI 线程抓取不可变草稿和预期版本；工作者不能读取/修改 QWidget。
- [x] 单次提交期间禁用对应保存/导入按钮，显示进度；结果通过 Qt signal 回到 GUI 线程。保留用户草稿，版本冲突明确显示，不能自动覆盖别人的更改。
- [x] 延迟返回的结果以请求序号和当前账号 ID 核对；用户切换选中账号后，旧保存结果不得写到新面板。窗口关闭后工作者结果不得访问已销毁控件。
- [x] 测试注入 500ms 写盘时仍能处理界面事件；断言工作线程不是 GUI 线程、完成通知回到 GUI 线程、连续点击只提交一次、失败保留草稿。
...
- [x] 将源码支持说明和 `setup.py` 的 Python 下界统一为 3.12；CI/安装器继续明确使用 3.12，其他版本不声称已经实测。
- [x] 在干净的隔离环境安装运行/开发依赖并执行 pip check，定位 PySide6 元包记录和残留 `~k-script` 的来源。若只是本机损坏，修复环境记录并说明，不把本机残留当成所有用户依赖错误；不删除未经确认的包目录。
- [x] 固定已经验证的直接依赖版本或构建约束，保持 requirements 输入、打包实际锁定结果一致；升级框架和大批 OCR 依赖分开实施，避免与账号修复混为一次变更。
- [x] `更新日志.md` 作为界面和发布说明的共同内容源。About 通过已有路径解析读取随包文件，使用 Qt 的 Markdown/文本显示；删除内嵌的重复长文本和无条件 return 后的死代码。
- [x] 更新 `validate_release.py`：验证 config 版本对应更新日志的明确版本条目，验证界面从共同来源读取；不再要求 About 源码包含某个 `Vx` 字面量。
- [x] 把 `更新日志.md`、完整 `custom_ok/` 和对应 framework 固定版本纳入两条打包清单。手工增量包使用受版本控制的覆盖文件，不直接从本机 `.venv` 取修改过的框架文件。
- [x] 增量包从上一支持版本更新后，与同版本全新安装比较关键源码/覆盖文件 SHA256；缺文件或不一致时不能发布该更新包。
...
**执行状态（2026-09-07）：** 离线验证与修复已完成，逐项证据见[整改执行记录](../../reviews/2026-09-06整改执行记录.md)。T15 的安装器完整生命周期和本表最后的真实游戏矩阵仍未完成，保留为待验收项。
