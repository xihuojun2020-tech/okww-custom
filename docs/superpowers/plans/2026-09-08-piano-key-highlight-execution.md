...
> **2026-09-09 `v1.42.09` 最终实现（最高优先级）：** 当前玩法按单键序列处理，不支持和弦输入。检测器在多个视觉候选中只接受唯一明显领先者：第一名与第二名的差值至少达到 ON/OFF 迟滞带宽 `0.015` 时输出第一名；候选接近时返回 `ambiguous`。明确 ON 键不会再被其他无关 UNCERTAIN 位置阻塞。任务仍使用两帧确认、按后 `0.15` 秒停顿、转场等待和手动停止。

> `PianoTeachingTask` 仅在检测状态变化时记录 `status/valid_dots/on/uncertain/top/reason`，避免逐帧刷日志并为 NAS 校准保留证据。`v1.42.09` 的 15 项弹琴测试及 2560×1440 参考图回放通过；提交 `907c08d5`、标签 `v1.42.09` 已推送。本文后续关于多键集合、组合按下、自动结束、OCR、参数 JSON 和大规模样本集的内容均为历史或候选增强项，不属于当前生产行为。
...
**Goal:** 将原设计落成可离线验证的 21 键高亮识别器、单键去重状态机和 ok-script 任务适配层；从可能包含动画残影或背景伪候选的画面中输出唯一明显领先的真实按键，不确定时停止输入。

**Architecture:** `src/task/piano.py` 是不依赖 Qt、设备和账号的纯算法模块，统一处理比例布局、圆环亮度差、候选决策和时序；`PianoTeachingTask` 复用框架截图、输入、配置和诊断接口。离线测试直接调用同一算法和状态机，用伪输入器验证事件和释放顺序。

**Tech Stack:** Windows / Python 3.12；现有 ok-script、OpenCV、NumPy、标准库 dataclasses、enum、unittest、unittest.mock。无新增运行依赖。

**Spec:** [原始设计](2026-09-08-piano-key-highlight-detection.md)，以及用户 2026-09-09 最终确认：真实玩法任一时刻只有一个按键高亮，按下后才出现下一个；一段通常 1—7 次，中间可能有剧情或转场，任务持续到手动关闭。该最终规则取代 2026-09-08 的多键推测。
...
本节记录 `1.40.00` 当时的多键假设；该假设后来被实机规则推翻。当前不再把真实和弦时序作为验收项，生产行为以第 5 节的单键判定为准；独立 1080p 实拍、UI 缩放变化和动态帧分数分布仍待实机补充。

## 5. `1.41.07` 回归与 `1.42.09` 修复状态

`v1.40.00` 与 `v1.41.07` 的坐标、圆环掩模、`0.075/0.060` ON/OFF 阈值和 18/21 圆点门禁相同。检测层唯一会让旧版产生事件而新版整帧停止的新分支，是 `v1.41.07` 对 `len(on) > 1` 的无条件拒绝。它把“游戏只有一个真实高亮”错误等同于“图像永远只有一个过阈值候选”，动态光效中的弱伪候选因此阻断两帧确认。

`v1.42.09` 使用现有迟滞带宽作为最小领先差，不增加用户配置和新依赖：

```text
ON 候选按 score 降序排列
没有 ON，存在 UNCERTAIN  → ambiguous
只有一个 ON             → candidate(best)
多个 ON，best-second<.015 → ambiguous
多个 ON，best-second≥.015 → candidate(best)
```

状态机仍只接收零个或一个可执行键，竞争候选不会产生事件。任务状态变化日志示例：

```text
弹琴检测 status=ambiguous valid_dots=21/21 on=S,Y uncertain=S,Y top=S:0.100,Y:0.095,R:0.047 reason=competing highlights
弹琴检测 status=candidate valid_dots=21/21 on=S uncertain=- top=S:0.110,Y:0.081,R:0.047 reason=dominant highlight over Y
```

验证结果：`tests/TestPianoTeaching.py` 15 项通过；参考图输出 `candidate ('S',)`，前三名 `S=0.11028`、`Y=0.05497`、`R=0.04747`。这些结果不替代新版本动态实机验收；若 NAS 日志显示真实高亮与伪候选长期差值小于 `0.015`，应先用对应完整 16:9 原始帧复核光环采样，再调整判定参数。
