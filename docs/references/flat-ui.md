...
- 账号设置：身份只读、每日任务、清理体力、周本、其他设置、序列和维护操作分区。保存、丢弃、编辑提示在账号选择区域下方；周本记录在周本详情中。账号配置仍在确认保存后事务提交；拆分分组不改变周本先于普通体力任务执行的顺序。
...
使用本地 Python 执行 `scripts/render_flat_ui.py`，创建临时合成账号和无操作设备服务，渲染五个真实页面，包括实际 DailyTask、MultiAccountDailyTask、WeeklyBossTask 的配置控件，不运行游戏任务。活动、测试与实时功能使用显式标注的模拟条目，仅用于覆盖相同 UI 组件，不注册到正式程序。Windows 离屏渲染显式载入系统字体，避免离屏平台缺省字体显示为方框。
...
渲染通过 `OKWW_UI_EXPAND_ALL=1` 检查全部展开模式，未设置时检查默认收起模式；覆盖 100%、125%、150%、200% DPI 与两种页面宽度。1.46.00 输出在 `tests/output/ui-1.46`，包括新建账号和模板编辑窗。验收记录见 `docs/references/ui-disclosure-verification.md`。
