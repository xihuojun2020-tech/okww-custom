...
本文是 2026-09-14 的分批改造清单，末尾进展记录截至 1.74.00；表内“待实施”和验收状态是当时快照。查现行入口和已发布修复时，需同时核对 `config.py`、更新日志及对应任务源码，不能把此表当作 1.84.01 的覆盖清单。
...
深塔逐页返回、单次开启挑战、环境提示关闭和材料培养/背包入口完成稳定等待，修正角色列表标题图标干扰；保留并行1.73.01修复。24个文件269项测试通过。用户追加跳过战令、邮件、模拟领域和聚落专用入口，原三类跳过项不变。未迁移细节与实机限制不标完成，详见[本轮报告](../reviews/2026-09-14-navigation-rollout-1.74.00.md)。
