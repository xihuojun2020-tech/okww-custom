...
初稿：2026-08-30；本次按 1.84.01 核对。

## 数据分类

- 带星号手机号：账号切换的首要匹配依据。
- U…A 备用登录名：手机号不可见时的备用匹配依据。
- 游戏内特征码：游戏内唯一身份；1.61.00 起用于初露峥嵘首尾核验，不替代登录列表定位。原始读数只用于绑定及内存比对，不写入公开日志或完成证据元数据。
...
诊断压缩包在 `.173` NAS 的 `待分析/压缩包/` 与 `已检查/压缩包/` 流转。采集画面可能直接包含 UID、昵称等；文字脱敏不能遮挡截图，读取、报告和转交时应按原始敏感证据处理。完成报告登记后，已检查 ZIP 按 3 天保留；未检查 ZIP 按已验证上传后的 30 天保留，报告继续保留。详见 [NAS 诊断说明](../NAS诊断部署与读取说明.md)。
