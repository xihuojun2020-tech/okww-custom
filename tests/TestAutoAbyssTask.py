...
class TestAutoAbyssTask(unittest.TestCase):
    ...
    def test_click_start_challenge_skips_click_when_floor_is_already_loading(self):
        ...
        task=harness.task();task._abyss_environment_hint=lambda frame:harness.button
        task.click_box=Mock()
        self.assertFalse(AutoAbyssTask._click_start_challenge(task))
        task.click_box.assert_not_called()

    def test_click_start_challenge_requires_team_page_and_clicks_its_button(self):
        from tests import TestNavigationAdapter as nav_tests
        harness=nav_tests.TestNavigationAdapter();self.addCleanup(harness.doCleanups)
        task=harness.task();button=harness.button;button.name='开启挑战';task.click_box=Mock()
        task._abyss_environment_hint=lambda frame:button if task.click_box.called else None
        task.ocr=lambda *args,**kwargs:[SimpleNamespace(name='编辑队伍'),button]
        self.assertTrue(AutoAbyssTask._click_start_challenge(task))
